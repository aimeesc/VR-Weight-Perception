...
namespace exiii.Unity.SuperCharacterController
{
    public static class SuperCollider
    {
        public static bool ClosestPointOnSurface(Collider collider, Vector3 to, float radius, out Vector3 closestPointOnSurface, out bool isInternal)
        {
            isInternal = false;

            if (collider is BoxCollider)
            {
                closestPointOnSurface = SuperCollider.ClosestPointOnSurface((BoxCollider)collider, to, out isInternal);
                return true;
            }
            else if (collider is SphereCollider)
            {
                closestPointOnSurface = SuperCollider.ClosestPointOnSurface((SphereCollider)collider, to, out isInternal);
                return true;
            }
            else if (collider is CapsuleCollider)
            {
                closestPointOnSurface = SuperCollider.ClosestPointOnSurface((CapsuleCollider)collider, to, out isInternal);
                return true;
            }
            else if (collider is MeshCollider)
            {
                ...
                if (bsp != null)
                {
                    closestPointOnSurface = bsp.ClosestPointOn(to, radius, out isInternal);
                    return true;
                }
        ...
        public static Vector3 ClosestPointOnSurface(SphereCollider collider, Vector3 to, out bool isInternal)
        {
            isInternal = false;
            ...
            var magnitude = p.sqrMagnitude;
            p.Normalize();

            p *= collider.radius * collider.transform.localScale.x;

            if (magnitude <= p.sqrMagnitude)
            {
                isInternal = true;
            }
        ...
        public static Vector3 ClosestPointOnSurface(BoxCollider collider, Vector3 to, out bool isInternal)
        {
            isInternal = true;
            ...
            if ((halfSize.x <= local.x || local.x <= -halfSize.x) ||
        ...
        // Courtesy of Moodie
        public static Vector3 ClosestPointOnSurface(CapsuleCollider collider, Vector3 to, out bool isInternal)
        {
            isInternal = false;
            ...
            var magnitude = p.sqrMagnitude;
            p.Normalize();
            var targetMagnitude = (p * collider.radius).sqrMagnitude;
